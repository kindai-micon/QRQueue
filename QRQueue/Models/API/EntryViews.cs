...
namespace QRQueue.Models.API
{
    ...
    /// <summary>参加登録 409(既に参加済み)のレスポンス。復元用の電子券IDを同梱する</summary>
    ...
    /// <summary>POST /api/entry/checkin のレスポンス(チェックイン)</summary>
    public record CheckinResult(long GroupNumber, GroupStatus Status);

    /// <summary>GET /api/entry/group/{joinToken} のレスポンス(グループ情報)</summary>
